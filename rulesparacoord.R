...
rs <- rules[2412:2438]
plot(rs, method="scatter", shading = "lift",measure="support", engine='htmlwidget')
...
#'
#' @return a "ggplot" object for further modifications
#' @export 
#'
#' @examples
#' example("eclat")
#' rs <- rules[2412:2438]
#' rulesparacoord(rs)
#' rulesparacoord(rs, method="plotly")
rulesparacoord <- function(ruleset,method="ggplot2", shading = "lift",measure="support"){
...
plt <- plt +theme_classic()+
  ggtitle(paste("Parallel coordinates plot for",length(rs),"rules"))+
